...
class ImageAppContainer extends React.PureComponent {
    ...
    renderLargeImage() {
        const largeImage = this.state.loading ?
            <h2>LOADING</h2> :
            <LargeImage
                src={require(`../assets/large/${this.state.activeTemplate.image}`).default}
                alt={this.state.activeTemplate.image}
            />;
    ...
    renderButtons() {
        ...
        return (
            <div>
                <img
                    className={`nav-button ${leftIndex === 0 ? 'disabled' : ''}`}
                    src={previous}
                    alt='previous'
                    onClick={() => this.handleNavClick('previous')}
                />
                <img
                    className={`nav-button ${rightIndex === this.state.allTemplates.length - 1 ? 'disabled' : ''}`}
                    src={next}
                    alt='next'
                    onClick={() => this.handleNavClick('next')}
                />
            </div>
        );
    }
    ...
    renderThumbnailContainer() {
        const thumbnails = this.state.loading ?
            <h2>LOADING</h2> :
            <ThumbnailContainer
                activeThumb={this.state.activeTemplate.thumbnail}
                thumbnails={this.state.visibleThumbs}
